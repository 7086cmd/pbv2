...
────────────────────────────
An ElementalQuestion's content paragraph may contain:

  [Text(stem)]                           – simple question, no sub-parts
  [Text(stem), List([Para(a), Para(b)])] – multi-part question

The List element uses lowercase-alphabetic / right-parenthesis ordering by
default (i.e. (a), (b), (c) …), matching the most common exam convention.
"""
...
from schema import Content, ExtractedPage, ExtractedQuestion
...
def insert_text_element(session: Session, content: Content) -> int:
    """Insert a ``texts`` row and wrap it in an ``elements`` row; return element id."""
    text_id = insert_text(session, content)
    elem = Element(kind=ElementKindEnum.text, text_id=text_id)
    session.add(elem)
    session.flush()
    return elem.id  # type: ignore[return-value]
...
def insert_paragraph(session: Session, content: Content) -> int:
    ...
    *content*.  Returns the paragraph ``id``.
    """
    elem_id = insert_text_element(session, content)

    para = Paragraph()
    session.add(para)
    session.flush()

    session.add(ParagraphElement(paragraph_id=para.id, position=0, element_id=elem_id))

    return para.id  # type: ignore[return-value]


def insert_sub_questions_list(session: Session, sub_questions: list[Content]) -> int:
    """
    Insert a ``lists`` row (lowercase-alphabetic, right-parenthesis) whose items
    are one paragraph per sub-question.  Returns the list ``id``.
    """
    lst = ListRow(
    ...
    for pos, sq_content in enumerate(sub_questions):
        para_id = insert_paragraph(session, sq_content)
...
def insert_content_paragraph(session: Session, q: ExtractedQuestion) -> int:
    """
    Build and insert the content ``paragraph`` for *q*.

    If *q* has no sub-questions the paragraph contains a single Text element.
    If *q* has sub-questions the paragraph contains:
      position 0 – Text element  (the stem)
      position 1 – List element  (the sub-question parts)
    """
    stem_elem_id = insert_text_element(session, q.content)

    para = Paragraph()
    session.add(para)
    session.flush()

    session.add(ParagraphElement(paragraph_id=para.id, position=0, element_id=stem_elem_id))

    if q.sub_questions:
        sub_contents = [sq.content for sq in q.sub_questions]
        list_id = insert_sub_questions_list(session, sub_contents)
        list_elem = Element(kind=ElementKindEnum.list, list_id=list_id)
        session.add(list_elem)
        session.flush()
        session.add(ParagraphElement(paragraph_id=para.id, position=1, element_id=list_elem.id))
...
def insert_elemental_question(
    ...
    content_para_id = insert_content_paragraph(session, q)
...
def store_page(
    ...
    """
    Persist all questions from one LLM-extracted page as ``single_problems``.
    ...
    for q in page.problems:
        q_id = insert_elemental_question(session, q, id_prefix, _used_ids=used_ids)
        ep = ElementalProblem(kind=ElementalProblemKindEnum.question, question_id=q_id)
        session.add(ep)
        session.flush()
        sp = SingleProblem(problem_id=ep.id, category_id=category_id)
        session.add(sp)
        session.flush()
